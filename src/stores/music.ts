...
export const useMusicStore = defineStore('music', () => {
    ...
    async function initPlayer(container: HTMLElement) {
        ...
        try {
            // localStorage缓存逻辑
            const cacheKey = `musicData_${server}_${type}_${id}`
            ...
            const now = Date.now()
            if (cache && Array.isArray(cache.data) && now - cache.time < 3600_000) {
                musicData.value = cache.data
            } else {
                const response = await axios.get(`https://api.injahow.cn/meting/?server=${server}&type=${type}&id=${id}`)
                musicData.value = response.data
                // 写入缓存
                localStorage.setItem(cacheKey, JSON.stringify({ data: musicData.value, time: now }))
            }
